...
Write to `~/.agent/diagrams/` with a descriptive filename and open the result in the browser. In Pi package installs, this is an explicit visual request: use `visual_explainer.prepare` when planning helps, then `visual_explainer.render` with a title, freeform semantic `bodyHtml`, and only necessary `customCss`.
